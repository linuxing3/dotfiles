...
*NOTE:* Curiously, it does not work if I change to =develop= branch, something wrong with the =xrandr= settings
...
#+begin_src shell :tangle ./exwm/EXWM.desktop :mkdirp yes
  ...
  Exec=sh /home/vagrant/.scratch.emacs.d/exwm/start-exwm.sh
  TryExec=sh
  Type=Application
...
Since people rarely use the =Right Ctrl= key and =CapsLock= key is big and neare,  the =Xmodmap= file will be used with the =xmodmap= program to remap =CapsLock= to =Right Ctrl= , and =Right Ctrl= to =CapsLock= inside of our desktop environment:
