...
;;         (url "file:///home/vagrant/workspace/code/guix")
;;         (introduction
;;           (make-channel-introduction
...
sudo -E guix system -L ~/.dotfiles/.config/guix/systems reconfigure ~/.dotfiles/.config/guix/systems/vagrant.scm
...
#+begin_src emacs-lisp :tangle .guix.emacs.d/per-system-settings.el :noweb yes
...
=vagrant= is a VIT P2423 laptop that I use at my day job.

*.config/guix/systems/vagrant.scm:*

#+begin_src scheme :tangle .config/guix/systems/vagrant.scm
...
#+begin_src sh :tangle .bin/update-channels :shebang #!/bin/sh
  ...
  guix pull --channels=~/.dotifiles/.config/guix/base-channels.scm
  guix describe --format=channels > ~/.dotfiles/.config/guix/channels.scm
...
Underhook, it use =guix package= to install packages

#+begin_src sh

update-profiles emacsslim
...
This script accepts a space-separated list of manifest file names (without extension) under the =~/.config/guix/manifests= folder and then =installs=  or =activate= those profiles for the first time.  For example:
...
just put everything of =.profile= in the =~/.dotfiles/custom/bash/plugin/guix/guix.plugin.sh=

And you are reay to go!

In my system, =emacs28= is installed in =.guix-extra-profiles/emacsslim/emacsslim/bin=

And the =guix-plugin= will add it to path

*** Local and Fonts

#+begin_src shell
guix install glibc-locales
export GUIX_LOCPATH=$HOME/.somepath/lib/locale
guix install fontconfig
fc-cache -rv
guix install font-hack font-fira-code
#+end_src
...
#+begin_src sh :tangle .bin/update-dotfiles :shebang #!/bin/sh
  ...
  emacs -Q --script ./.guix.emacs.d/tangle-dotfiles.el

  # Make sure any running Emacs instance gets updated settings
  emacsclient -e '(load-file "~/.guix.emacs.d/per-system-settings.el")' -a "echo 'Emacs is not currently running'"
...
*.guix.emacs.d/tangle-dotfiles.el*

#+begin_src emacs-lisp :tangle .guix.emacs.d/tangle-dotfiles.el
...
*.guix.emacs.d/lisp/dw-settings.el*

#+begin_src emacs-lisp :tangle .guix.emacs.d/lisp/dw-settings.el
...
#+begin_src sh
  ...
  git clone https://github.com/linuxing3/dotfiles-1
